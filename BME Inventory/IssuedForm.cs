    ...
    public partial class Distribute : Form
    {
        ...
        private void update_btn3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "UPDATE inventory SET stock = stock - @stock WHERE page_no = @page_no";
                using (SqlCommand cmd = new SqlCommand(query, dbManager.GetConnection()))
                {
                    ...
                    {
                        cmd.Parameters.AddWithValue("@stock5", stockValue);

                        int rowsAffected = cmd.ExecuteNonQuery();

                        if (rowsAffected > 0)
                        {
                            string logMessage = $"{user_lbl_distribution.Text} updated record with page No. {page_no_txt.Text} successfully with a increase of {stockValue} from the stock value of {stock_lbl.Text} at {DateTime.Now}";
