...
namespace Acc_Inventory
{
    public partial class Database : Form
    {
        ...
        {
            string query = "SELECT page_no, item_name, item_cat, stock FROM inventory WHERE stock < lower";
        ...
        {
            string query = "SELECT page_no, item_name, item_cat, stock FROM inventory";
            DataTable table = new DataTable();
            LoadDataIntoGrid(query, table);
        }
        ...
        private void mail_btn_Click(object sender, EventArgs e)
        {
            ...
            if (table.Rows.Count > 0)
            {
                ...
                string FilePath = Path.Combine(FolderPath, $"StoreData.xlsx");

                using (var package = new ExcelPackage(new FileInfo(FilePath)))
                {
                    string worksheetName = "Store Data";
                    int worksheetNumber = 1;

                    while (package.Workbook.Worksheets.Any(ws => ws.Name == worksheetName))
                    {
                        worksheetName = "Store Data " + worksheetNumber;
                        worksheetNumber++;
                    }
        ...
        {
            string query = "SELECT page_no, item_name, item_cat, upper, lower, stock FROM inventory";
        ...
        private void search_btn_database_Click(object sender, EventArgs e)
        {
            ...
            string[] columnsToSearch = { "page_no", "item_name", "item_cat"};
